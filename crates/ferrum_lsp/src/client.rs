...
use lsp_types::{
    ...
    request::{
        Completion, GotoDefinition, HoverRequest, Initialize, References, DocumentSymbolRequest,
    },
    ...
    VersionedTextDocumentIdentifier, DocumentSymbolParams, DocumentSymbolResponse,
};
...
impl LspClient {
    ...
    pub async fn completion(&self, uri: Uri, position: lsp_types::Position) -> Result<Vec<CompletionItem>> {
    ...
    pub async fn document_symbols(&self, uri: Uri) -> Result<DocumentSymbolResponse> {
        ...
        let response: Option<DocumentSymbolResponse> = self.request::<DocumentSymbolRequest>(params).await?;
    ...
    {
        self.request_raw(R::METHOD, serde_json::to_value(params).unwrap()).await
    }
