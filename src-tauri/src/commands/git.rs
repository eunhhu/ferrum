...
pub async fn git_stage(repo_path: String, file_path: String) -> Result<(), String> {
    ...
    let mut index = inner.index().map_err(|e| format!("Failed to get index: {}", e))?;
    index
...
pub async fn git_unstage(repo_path: String, file_path: String) -> Result<(), String> {
    ...
    let head = inner.head().map_err(|e| format!("Failed to get HEAD: {}", e))?;
    ...
    inner
        .reset_default(Some(&head_commit.as_object()), [std::path::Path::new(&file_path)])
...
pub async fn git_log(repo_path: String, limit: Option<usize>) -> Result<Vec<GitCommitInfo>, String> {
    ...
    let mut revwalk = inner.revwalk().map_err(|e| format!("Failed to create revwalk: {}", e))?;
    revwalk
        .push_head()
...
/// Get file diff
#[tauri::command]
pub async fn git_diff_file(repo_path: String, file_path: String, staged: bool) -> Result<String, String> {
    let repo = Repository::open(&repo_path).map_err(|e| e.to_string())?;
    let inner = repo.inner();
